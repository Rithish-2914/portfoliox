...
import { projectCategories, updateProjectCodeSchema } from "@shared/schema";
...
export async function registerRoutes(
  ...
  app.patch("/api/projects/:id/code", async (req, res) => {
    try {
      ...
      const parseResult = updateProjectCodeSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid request body", errors: parseResult.error.errors });
      }

      const project = await storage.updateProjectCode(id, parseResult.data);
      if (!project) {
        return res.status(404).json({ message: "Project not found" });
      }
      ...
      res.status(500).json({ message: "Failed to update project code" });
    }
  });
